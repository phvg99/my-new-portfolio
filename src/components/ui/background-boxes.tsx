"use client";
import React, { useCallback } from "react";
import { motion, useMotionValue } from "motion/react";
import { cn } from "@/lib/utils";

const COLORS = [
  "rgb(125 211 252)", // sky-300
  "rgb(249 168 212)", // pink-300
  "rgb(134 239 172)", // green-300
  "rgb(253 224 71)", // yellow-300
  "rgb(252 165 165)", // red-300
  "rgb(216 180 254)", // purple-300
  "rgb(147 197 253)", // blue-300
  "rgb(165 180 252)", // indigo-300
  "rgb(196 181 253)", // violet-300
];

function BoxCell({ showPlus }: { showPlus: boolean }) {
  const bgColor = useMotionValue("transparent");

  const handleHoverStart = useCallback(() => {
    bgColor.set(COLORS[Math.floor(Math.random() * COLORS.length)]);
  }, [bgColor]);

  const handleHoverEnd = useCallback(() => {
    bgColor.set("transparent");
  }, [bgColor]);

  return (
    <motion.div
      style={{ backgroundColor: bgColor }}
      onHoverStart={handleHoverStart}
      onHoverEnd={handleHoverEnd}
      className="w-16 h-8 border-r border-t border-neutral-800 relative"
    >
      {showPlus ? (
        <svg
          xmlns="http://www.w3.org/2000/svg"
          fill="none"
          viewBox="0 0 24 24"
          strokeWidth="1.5"
          stroke="currentColor"
          className="absolute h-6 w-10 -top-[14px] -left-[22px] text-neutral-800 stroke-[1px] pointer-events-none"
        >
          <path
            strokeLinecap="round"
            strokeLinejoin="round"
            d="M12 6v12m6-6H6"
          />
        </svg>
      ) : null}
    </motion.div>
  );
}

const MemoizedBoxCell = React.memo(BoxCell);

export const BoxesCore = ({ className, ...rest }: { className?: string }) => {
  const rows = new Array(30).fill(1);
  const cols = new Array(20).fill(1);

  return (
    <div
      style={{
        transform: `translate(-40%,-60%) skewX(-48deg) skewY(14deg) scale(0.675) rotate(0deg) translateZ(0)`,
      }}
      className={cn(
        "absolute left-1/4 p-4 -top-1/4 flex -translate-x-1/2 -translate-y-1/2 w-full h-full z-0",
        className
      )}
      {...rest}
    >
      {rows.map((_, i) => (
        <motion.div
          key={`row` + i}
          className="w-16 h-8 border-l border-neutral-800 relative"
        >
          {cols.map((_, j) => (
            <MemoizedBoxCell key={`col` + j} showPlus={j % 2 === 0 && i % 2 === 0} />
          ))}
        </motion.div>
      ))}
    </div>
  );
};

export const Boxes = React.memo(BoxesCore);
